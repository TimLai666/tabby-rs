...
const settingsSource = fs.readFileSync(path.join(root, 'tabby-settings/src/components/settingsTab.component.ts'), 'utf8')
...
const manualCheckMethod = settingsSource.match(/async checkForUpdates \(\) \{([\s\S]*?)\n    \}/)?.[1]
assert.ok(manualCheckMethod, 'manual update check method must exist')
assert.match(manualCheckMethod, /try \{[\s\S]*?await this\.updater\.check\(\)[\s\S]*?\} finally \{[\s\S]*?checkingForUpdate = false/)
