...
async function smokeDeb (artifact, root, operations) {
    ...
    await assertCommand('dpkg', ['--force-not-root', `--root=${packageRoot}`, `--admindir=${adminDirectory}`, `--instdir=${packageRoot}`, '--unpack', artifact], { cwd: root })
    ...
    await assertCommand('dpkg', ['--force-not-root', `--root=${packageRoot}`, `--admindir=${adminDirectory}`, `--instdir=${packageRoot}`, '--purge', name], { cwd: root })
