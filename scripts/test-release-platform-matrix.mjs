...
const linuxBundleDependencies = buildSteps.find(step => step.name === 'Install Linux build dependencies')
...
assert.match(linuxDependencies?.run || '', /desktop-file-utils/, 'release Web gate must install desktop-file-utils for Linux deep-link registration')
...
assert.match(linuxBundleDependencies?.run || '', /desktop-file-utils/, 'release bundle jobs must install desktop-file-utils for Linux deep-link registration')
