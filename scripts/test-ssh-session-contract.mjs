...
const windowsAgent = engine.match(/#\[cfg\(windows\)\]\s+let mut agent = match socket \{([\s\S]*?)\n        \};/)
assert.ok(windowsAgent, 'Windows SSH agent routing contract is missing')
assert.match(windowsAgent[1], /connect_named_pipe\(path\)/)
assert.match(windowsAgent[1], /connect_pageant\(\)/)
assert.doesNotMatch(windowsAgent[1], /let _ = socket/)
