...
assert.match(engine, /let mut agent = super::connect_agent\(socket\.map\(str::to_owned\)\)\.await\?;/)
const connectAgent = rust.match(/async fn connect_agent\([\s\S]*?\n\}\n\nfn validate_request/)
assert.ok(connectAgent, 'SSH agent connection helper is missing')
const windowsAgent = connectAgent[0].match(/#\[cfg\(windows\)\][\s\S]*?return Ok\(client\);[\s\S]*?\n    \}/)
assert.ok(windowsAgent, 'Windows SSH agent routing contract is missing')
assert.match(windowsAgent[0], /connect_named_pipe\(path\)/)
assert.match(windowsAgent[0], /connect_pageant\(\)/)
