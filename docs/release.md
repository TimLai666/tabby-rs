...
For cross-platform parity or release-gate investigation, start the manual workflow with `evidence_only` enabled. It still builds and uploads the platform and aggregate reports, preserves every failed gate in JSON, and skips publishing. This mode does not waive manual checks or provide signing credentials, so the protected release environment still requires its normal approval and secrets.
